...
const renderPretty = (params) => {
    ...
    if (connectionString) {
        ...
        if (!showCredentials && connectionStringHasCredentials(connectionString)) {
            secondary(`To reveal the full connection string (including credentials), run ${formatCommand('database status --show-credentials')}`);
        }
        else {
    ...
    if (status.missingOnDisk.length > 0 || status.outOfOrder.length > 0) {
        ...
        if (status.outOfOrder.length > 0) {
            log(`  Out of order: ${status.outOfOrder
                ...
                .join(', ')}`);
            log('');
            log(chalk.gray(`Run ${formatCommand('database migrations reset')} to delete these local-only migrations, then generate them again with a higher prefix.`));
        }
    }
};
